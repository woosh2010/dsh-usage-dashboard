...
// ────────────────────────────────────────────────────────────────
// Pricing — mirrors lib/client.js (PRICE_TABLE / periodAt / stepCost).
 ...
 * @param options - `{ session, model, turnLimit }` filters. `modelsAll` (the
 *   per-model
 *   aggregates used for the model filter dropdown and the token-mix selector)
 *   is computed over the session/time range BEFORE the model filter, while
 *   every other aggregate applies all filters.
...
function aggregateSummary(records, since = 0, recentLimit = 20, sessionTitles = void 0, options = {}) {
	...
	const inRange = records.filter((record) => record.time >= since && (sessionFilter === void 0 || record.sessionId === sessionFilter));
	...
	// Recent list: when `turnLimit` is set, keep every step of the latest N
	// distinct turns (walking backwards from the newest record); otherwise
	// keep the last `recentLimit` steps.
	let recentRecords;
...
function createRoutes(ctx, options) {
	...
	const handleSummary = (req, res) => {
		...
		try {
			...
			send(res, { ok: true, value: aggregateSummary(records, since, limit, readSessionTitles(home), { session, model, turnLimit: turns }) });
		} catch (error) {
...
export { aggregateSummary, appendRecords, apply, createRoutes, historyPath, inject, loadRecords, name, parseRecord, readProjectionModels, readSessionTitles, repairUnknownModels, shortSessionId, turnModelsProjectionDefinition };
